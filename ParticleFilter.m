...
for k = 2 : sensor_data
    ...
    %模拟一个弧线运动的状态
    X(:, k) = X(:, k-1) + ZIN(4,k)*4 * [(sin( ZIN(3,k))); cos(ZIN(3,k))] + wgn(2, 1, 10*log10(Q));     %状态方程
    Z(:, k) = ZIN(1:2, k) + wgn(2, 1, 10*log10(R));     %观测方程 
   
    %粒子滤波
    %预测
    for i = 1 : N
        P(:, i) = P(:, i) + ZIN(4,k) * [sin( ZIN(3,k)); cos( ZIN(3,k))] + wgn(2, 1, 10*log10(Q));
...
figure(3);
set(gca,'FontSize',12);
plot( Z(1,:), Z(2,:), '>', PCenter(1,:), PCenter(2,:), 'g');
axis([cordinatex-100 cordinatex+200 cordinatey-200 cordinatey+100]),grid on;
legend('观测轨迹', '粒子滤波轨迹');
